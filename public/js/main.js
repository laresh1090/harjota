...
const allDropmenu  = document.querySelectorAll(".switchA");

/* ========================================================== */
function getPathAfterSlash(inputString) {
    // Count the number of slashes in the string
    const slashCount = (inputString.match(/\//g) || []).length;
    
    // If there are fewer than 3 slashes, return '/'
    if (slashCount < 3) {
        return '/';
    }
    
    // Find the index of the last slash and return substring from there
    const lastSlashIndex = inputString.lastIndexOf('/');
    return inputString.substring(lastSlashIndex);
}

function getPathAfterSlash001(inputString) {
    // Count the number of slashes in the string
    const slashCount = (inputString.match(/\//g) || []).length;
    
    // If there are fewer than 2 slashes, return '/'
    if (slashCount < 2) {
        return '/';
    }
    ...
    // Find the index of the second-to-last slash by searching backwards from lastSlashIndex
    const secondToLastSlashIndex = inputString.lastIndexOf('/', lastSlashIndex - 1);
    
    // Return substring from second-to-last slash
    return inputString.substring(secondToLastSlashIndex);
}
...
for (let index = 0; index < allmenu.length; index++) {
	...
	console.log(allmenu_new1);
	
    allmenu_new2=getPathAfterSlash(allmenu_new1);

	console.log(allmenu_new2);
    if (allmenu_new2==switch_location) {
        allmenu_new.classList.add("activate");
    }    
}

for (let index = 0; index < allDropmenu.length; index++) {
    _allmenu_new=allDropmenu[index];
	console.log(_allmenu_new);
	
    _allmenu_new1 = _allmenu_new.getAttribute("href");
	__allmenu_new1 = _allmenu_new.parentElement;
	___allmenu_new1 =__allmenu_new1.parentElement;
	____allmenu_new1 = ___allmenu_new1.parentElement;
	____allmenu_new2=____allmenu_new1.firstElementChild;
	console.log(____allmenu_new2);
	
	console.log(_allmenu_new1);
	
    _allmenu_new2=getPathAfterSlash001(_allmenu_new1);

	console.log(_allmenu_new2);
    if (_allmenu_new2==switch_location) {
        _allmenu_new.classList.add("activate");
		____allmenu_new2.classList.add("activate");
    }    
}
